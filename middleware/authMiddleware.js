...
const { verifyToken, verifyResetToken } = require("../utils/handleJWT");
...
const authMiddlewareReset = async (req, res, next) => {
    try{
        if (!req.headers.authorization) {
            handleHttpError(res, "NOT_TOKEN", 401)
            return
        ...
        const token = req.headers.authorization.split(' ').pop() 
        //Del token, miramos en Payload (revisar verifyToken de utils/handleJwt)
        const dataToken = await verifyResetToken(token)
...
module.exports = {authMiddleware, authMiddlewareReset}